...
class GSMTApp {
    ...
    async handleGlobal24HAnalysis() {
        ...
        try {
            ...
            this.showToast('24H analysis failed. Using demo data.', 'error');
            
            // Fallback to demo data
            const demoData = this.generateDemo24HData();
            ...
            this.state.marketHours = demoData.market_hours;
            this.update24HChart();
            this.updateMarketSessions();
            this.updatePerformanceSummary();
        } finally {
    ...
    handleAnalysisModeChange() {
        ...
        if (analysisMode === 'global-24h') {
            // Clear current selection and auto-analyze global markets
            this.state.selectedSymbols.clear();
            this.updateSelectedSymbolsDisplay();
            this.showToast('Switched to Global 24H Market Flow mode', 'info');
        } else {
            // Standard mode - clear any global data
            this.state.chartData.clear();
            ...
            this.showToast('Switched to Standard Analysis mode', 'info');
        }
    }
